...
let
  ...
  inherit (osConfig.sys.tmpfiles.lib) toMode toText trim;
  inherit (config.home) homeDirectory username;
  c = config.sys.tmpfiles;
in {
  options.sys.tmpfiles = {
  ...
  config.systemd.user.tmpfiles.rules =
    (
      ...
      in
        rulesFor directory) (unique c.directories)
    )
    ++ (
    ...
    ++ (
      ...
      map (x: let
        ...
        rulesFor = {
          ...
          '';
      in
        rulesFor symlink) (unique c.symlinks)
    );
}
